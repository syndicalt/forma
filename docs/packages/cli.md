...
secret hygiene, release file presence, package CI coverage, publish bundle
coverage, and eval suite summary. It prints a machine-readable checklist
...
and host examples for both runtimes, omits the package README or scaffolded CI
workflows from `releaseFiles`, omits package-check, lock check, eval-suite, or
package-review commands from the package CI workflow, omits reviewed artifacts
from the publish workflow bundle, or when the eval suite does not cover every
