...
`publish-bundle`, `eval-coverage`, and `eval-suite` as passed, with
...
workflows are present in the reviewed artifact set. Publish bundle review
checks that the publish workflow references every reviewed artifact path before
building the release bundle. Provider profile review fails if the profile embeds
an `apiKey`; agent task packages should include a provider profile that uses
