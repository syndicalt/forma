...
Cleaned-up route labels should preserve host override ownership. If a
deployment override supplied the original label, keep the cleaned-up label in
that override path until review promotes it into the shared provider profile.
Cleaned-up route labels should preserve deployment audit keys. Keep adapter
logs, retry records, and review notes addressable by the original host-owned
route key, then attach the cleaned-up label as display metadata for audit
readers.
Cleaned-up route labels should preserve override audit lookup keys. When a host
override renames a route, keep override logs and adapter configuration
searchable by the original override key, then attach the cleaned-up label as
display metadata.
Cleaned-up route labels should preserve provider audit lookup keys. Keep
...
Cleaned-up route labels should preserve provider-attempt audit lookup keys.
Keep provider attempts, adapter logs, retry evidence, and reviewed-default notes
searchable by the original provider-attempt route key, then attach the
cleaned-up label after the provider attempt is recorded.
Cleaned-up route labels should preserve provider-result audit lookup keys.
Keep provider results, adapter logs, retry evidence, and reviewed-default notes
searchable by the original provider-result route key, then attach the
cleaned-up label after the provider result is stored.
...
host ownership and review boundaries are recorded.
Cleaned-up route labels should preserve fallback audit lookup keys. Keep
...
Cleaned-up route labels should preserve fallback-proof audit lookup keys. Keep
fallback proof records, failed-result diagnostics, rerun traces, and adapter
notes searchable by the original fallback-proof route key, then attach the
cleaned-up label after the fallback proof is accepted.
...
cleaned-up label after the fallback result is saved.
...
Cleaned-up route labels should preserve adapter-review audit lookup keys. Keep
adapter review notes, provider override evidence, route-change decisions, and
deployment-owner approvals searchable by the original adapter-review route key,
then attach the cleaned-up label after adapter review accepts the change.
...
Fallback diagnostics should be logged before route changes. Capture the failed
`FormaResult` diagnostics while the original provider context is still known,
then switch routes only after that evidence is available to the host workflow.
