...
release-proof bundles, package-review notes, uploaded summaries, and rerun
evidence searchable by the original release-proof route key, then attach the
cleaned-up label after the proof run is accepted.
Cleaned-up route labels should preserve release-proof-decision audit lookup keys.
Keep release-proof decisions, package-review notes, uploaded summaries, and
rerun evidence searchable by the original release-proof-decision route key,
then attach the cleaned-up label after the release proof decision is recorded.
Cleaned-up route labels should preserve proof-result audit lookup keys. Keep
proof results, release summaries, package-review notes, and rerun evidence
searchable by the original proof-result route key, then attach the cleaned-up
label after the proof result is accepted.
Cleaned-up route labels should preserve proof-decision audit lookup keys. Keep
proof decisions, release summaries, package-review notes, and rerun evidence
searchable by the original proof-decision route key, then attach the
cleaned-up label after the proof decision is recorded.
...
Cleaned-up route labels should preserve release-attempt audit lookup keys. Keep
...
Cleaned-up route labels should preserve artifact-proof audit lookup keys. Keep
artifact proof notes, release bundles, uploaded summaries, and saved proof logs
searchable by the original artifact-proof route key, then attach the cleaned-up
label after the artifact proof is accepted.
Cleaned-up route labels should preserve artifact-decision audit lookup keys.
Keep artifact decisions, release bundles, uploaded summaries, and saved proof
logs searchable by the original artifact-decision route key, then attach the
cleaned-up label after the artifact decision is recorded.
...
route key, then attach the cleaned-up label after the installed proof is
accepted.
Cleaned-up route labels should preserve triage audit lookup keys. Keep triage
...
label after the triage result is saved.
Cleaned-up route labels should preserve triage-decision audit lookup keys.
Keep triage decisions, failure summaries, package-kind markers, and recovery
commands searchable by the original triage-decision route key, then attach the
cleaned-up label after the triage decision is recorded.
Cleaned-up route labels should preserve failure-summary audit lookup keys. Keep
installed smoke failure summaries, failed package-kind markers, saved logs, and
recovery notes searchable by the original failure-summary route key, then add
the cleaned-up label after the failure summary is saved.
Cleaned-up route labels should preserve failure-result audit lookup keys. Keep
installed smoke failure results, failed package-kind markers, saved logs, and
recovery notes searchable by the original failure-result route key, then attach
the cleaned-up label after the failure result is saved.
Cleaned-up route labels should preserve failure-attempt audit lookup keys. Keep
installed smoke failure attempts, failed package-kind markers, saved logs, and
recovery notes searchable by the original failure-attempt route key, then
attach the cleaned-up label after the failure attempt is recorded.
Cleaned-up route labels should preserve failure-decision audit lookup keys.
Keep installed smoke failure decisions, failed package-kind markers, saved
logs, and recovery notes searchable by the original failure-decision route key,
then attach the cleaned-up label after the failure decision is recorded.
